...
if stock_name:
    try:
        with st.spinner(f"'{stock_name}' 주가 데이터를 수집하고 있습니다..."):
            # 1. 종목코드 찾기 (FinanceDataReader 이용)
            ...
            if target.empty:
                ...
                market = target.iloc[0]['Market'] # KOSPI or KOSDAQ
                ...
                if len(df) < 60:
                    st.warning("데이터가 부족하거나 가져올 수 없습니다. (상장폐지 또는 티커 오류)")
                else:
                    # 야후 데이터는 컬럼이 MultiIndex일 수 있어서 정리
                    ...
                    st.metric(label=f"{stock_name} ({code}) 현재가", 
